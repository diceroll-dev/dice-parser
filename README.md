...
| Label                         | `<term>[<label>]` or `(<expression>)[<label>]`      | `1d8[slashing] + 3[STR]` | `[...]` is literal syntax: square brackets only ever mean a label. Tags a die, number or group without changing the total; the label is the last suffix of a term (`4d6k3[STR]`) and is exposed as `label` on the expression and its `ResultTree`. Surrounding whitespace is trimmed and `[]` (or only spaces) means no label. A label is at most 64 Unicode code points of letters, combining marks, decimal digits, symbols and emoji (e.g. `(2d8 +2)[🔥 sword]`), spaces and `- _ ' . , : / + ( ) # & ! ?`; anything else (e.g. `*`, `<`, `` ` ``, `^`, tabs, invisible or bidi control characters) is a parse error. Labels and `ParseException` messages (which quote the input verbatim) are not escaped: escape or validate them before rendering, logging, or using them as identifiers or paths |
